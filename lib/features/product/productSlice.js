        ...
        try {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    console.warn('[Redux] Could not parse error response as JSON:', parseError)
                }
                
                console.error(`[Redux] Products API returned ${response.status}:`, errorData)
                throw new Error(errorData?.error || errorData?.message || `Failed to fetch products (${response.status})`)
            }
            ...
            try {
                ...
                console.error('[Redux] Failed to parse API response as JSON:', parseError)
            ...
            // If it's a network error or API unavailable, try fallback in development
            if (isDevelopment && (error?.message?.includes('fetch') || error?.message?.includes('Network'))) {
                console.warn('[Redux] Network error, using fallback data in development')
                return normalizeFallbackProducts(productDummyData)
            }
