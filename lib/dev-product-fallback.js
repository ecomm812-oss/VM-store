...
export function shouldAllowDevProductFileFallback() {
  ...
  return isDevelopment() && (!databaseUrl || databaseUrl.includes('localhost:5432/vmstore') || databaseUrl.includes('neon.tech'))
}
...
async function readDevProducts() {
  try {
    ...
    return Array.isArray(productDummyData) ? productDummyData : []
  } catch (error) {
