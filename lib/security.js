...
export function shouldUseDatabaseFallback(error) {
...
export async function getOrCreateUserRecord(options = {}) {
  ...
  const normalizedProvidedClerkId = typeof providedClerkId === 'string' ? providedClerkId.trim() : ''
  const resolvedAuthUserId = normalizedProvidedClerkId ? null : await getAuthUserId()
  const clerkId = normalizedProvidedClerkId || resolvedAuthUserId || ''
  ...
  const normalizedFallbackEmail = typeof fallbackEmail === 'string' ? fallbackEmail.trim().toLowerCase() : ''
  const resolvedEmail = getUserPrimaryEmail(clerkUser, normalizedFallbackEmail, clerkId)
