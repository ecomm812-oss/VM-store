...
export async function getLatestProducts({ take = 4 } = {}) {
  try {
    const products = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Connection terminated due to connection timeout')), 3500))
    ])
...
export async function getStoreWithProducts(username) {
  const storeUsername = typeof username === 'string' ? decodeURIComponent(username).trim() : ''
  if (!storeUsername) return null
  ...
  try {
    ...
    return {
      ...
      logo: toImageSrc(store.logo),
      products: normalizeProductList(store.Product)
    }
  } catch (error) {
    if (isDatabaseTimeoutError(error) || isProductColumnTypeMismatch(error)) {
      try {
        ...
        console.warn('getStoreWithProducts fallback failed:', getErrorMessage(fallbackError))
      }
    }

    if (shouldUseDevProductFallback(error)) {
      const store = await getDevStoreByUsername(storeUsername)
      if (!store) return null
