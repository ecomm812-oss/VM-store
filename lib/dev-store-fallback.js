...
export async function getAllDevStores() {
  return readDevStores()
}
...
export async function getDevStoreById(storeId) {
  const stores = await readDevStores()
  return stores.find(store => store.id === storeId) || null
}

export async function getDevStoreByUsername(username) {
  const stores = await readDevStores()
  const normalizedUsername = username.toLowerCase()
  return stores.find(store => store.username === normalizedUsername) || null
}
